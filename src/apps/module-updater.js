...
export default class ModuleUpdater extends Dialog 
{
            ...
            buttons:
            {
                update:
              {
                  ...
                  callback: html => 
                  {
                      ...
                      {
                          return ui.notifications.notify(game.i18n.localize("IMPMAL.UpdaterError"));
                      }
    ...
    {
        let html = await renderTemplate("systems/impmal/templates/apps/module-updater.hbs", module);
    ...
    async updateImportedContent(settings)
    {
        ...
        ui.notifications.notify(`${game.i18n.format("IMPMAL.UpdaterNotification", { created: this.count.created,  updated: this.count.updated,  name: this.data.module.id, version: this.data.module.version })}`);
        ...
        for (let document of documents)
        {
            ...
            if (game[document.collectionName].has(document.id))
            {
                ...
                if (!settings.excludeNameChange || (settings.excludeNameChange && document.name == existingDoc.name))
                {
                    ...
                    game.impmal.log(`Updated Document ${document.name}`);
                    this.count.updated++;
                }
            }
            else 
            {
                ...
                toCreate.push(newDoc);
                game.impmal.log(`Imported Document ${document.name}`);
                this.count.created++;
            }
        }
