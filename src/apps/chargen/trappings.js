import WFRP_Utility from "../../system/utility-impmal.js";
import Itemimpmal from "../../item/item-impmal.js";
import { ChargenStage } from "./stage";

export class TrappingStage extends ChargenStage {
  journalId = "Compendium.impmal-core.journals.JournalEntry.IQ0PgoJihQltCBUU.JournalEntryPage.hQipqLYlbBEjJEWL"
  static get defaultOptions() {
    ...
    options.title = game.i18n.localize("IMPMAL.CHARGEN.StageTrappings");
    return options;
  }

  static get title() { return game.i18n.localize("IMPMAL.CHARGEN.StageTrappings"); }

  get template() {
    return "systems/impmal/templates/apps/chargen/trappings.hbs";
  }

  constructor(...args) {
    super(...args);

    this.context.classStrings = game.impmal.config.classTrappings[this.data.items.career.system.class.value]?.split(",") || [];
    ...
    this.context.class = Promise.all(this.context.classStrings.map(i => WFRP_Utility.find(i.trim(), game.impmal.config.trappingItems)));
    this.context.career = Promise.all(this.context.careerStrings.map(i => WFRP_Utility.find(i, game.impmal.config.trappingItems)));
  }
  ...
  _updateObject(ev, formData) {
    ...
    let missing = this.context.missing.filter(i => i.choice == "keep").map(i => new Itemimpmal({ name: i.string, type: "trapping", system: { "trappingType.value": "misc" } }));
