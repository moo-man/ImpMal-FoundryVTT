...
export class SkillsTalentsStage extends ChargenStage {
  journalId = "Compendium.impmal-core.journals.JournalEntry.IQ0PgoJihQltCBUU.JournalEntryPage.f5Y4XenZVtDU2GUo"
  static get defaultOptions() {
    ...
    options.title = game.i18n.localize("IMPMAL.CHARGEN.StageSkillsTalents");
    return options;
  }

  static get title() { return game.i18n.localize("IMPMAL.CHARGEN.StageSkillsTalents"); }
  ...
  constructor(...args) {
    ...
    for (let [key, value] of Object.entries(randomTalents)) {
      let table = game.impmal.tables.findTable(key);

      if (!(table instanceof RollTable)) {
        ui.notifications.error(game.i18n.format("IMPMAL.CHARGEN.ERROR.TalentsTableNotFound", {key, species: this.data.species, subspecies: this.data.subspecies}));
        continue;
      }
  ...
  get template() {
    return "systems/impmal/templates/apps/chargen/skills-talents.hbs";
  }
    ...
    data.talents = {
      ...
      choices: this.context.speciesTalents.choices.map((choice, index) => {
        return choice.split(",").map(i => {
          ...
          if (match) {
            if (amount === 1)
              name = game.i18n.localize("IMPMAL.CHARGEN.AdditionalRandomTalent");
            else
              name = game.i18n.format("IMPMAL.CHARGEN.XAdditionalRandomTalents", { x: amount });
  ...
  activateListeners(html) {
    ...
    html.find(".reroll-duplicate").click(async ev => {
      ...
      let talent = await game.impmal.tables.rollTable(table.key);
